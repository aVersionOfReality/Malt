// Float math nodes for compute graphs.
// All dependencies are self-contained (no Common.glsl / render-pipeline variables).

#ifndef COMPUTE_FLOAT_MATH_GLSL
#define COMPUTE_FLOAT_MATH_GLSL

// ── Shared helpers ───────────────────────────────────────────────────────────
// Guarded so including both FloatMath and VectorMath in the same graph is safe.
...
/*META
    ...
    @to_max: default=1.0;
*/
float Float_map_range(bool clamped, float value,
                      float from_min, float from_max,
                      float to_min,   float to_max)
{
    return clamped
        ? _mrange_c(value, from_min, from_max, to_min, to_max)
        :  _mrange(value,  from_min, from_max, to_min, to_max);
}
